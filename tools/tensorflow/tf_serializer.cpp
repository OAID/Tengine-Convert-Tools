...
static bool LoadReduction(TFNode* tf_node, TFGraph& tf_graph, StaticGraph* graph)
{
    ...
    if (input1->static_tensor->dims.empty())
    {
        // printf("tensor dims is empty, force push back 1\n");
        input1->static_tensor->dims.push_back(1);
    }
...
static bool LoadMaximum(TFNode* tf_node, TFGraph& tf_graph, StaticGraph* graph)
{
    ...
    if (tf_node->inputs[1]->static_tensor->dims.empty())
        tf_node->inputs[1]->static_tensor->dims.push_back(1);
...
bool TFSerializerRegisterOpLoader(void)
{
    ...
    p_tf->RegisterOpLoadMethod("Square", op_load_t(LoadUnary));
