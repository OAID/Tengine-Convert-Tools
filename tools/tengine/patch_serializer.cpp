...
static struct model_patch *g_patch_list = NULL;
static unsigned int g_path_num = 0;
...
std::vector<struct model_patch> PatchSerializer::GetModelByName(const std::string& model_name)
{
    ...
    for(unsigned int i = 0; i < g_path_num; i++)
    {
...
{
     g_patch_list = (struct model_patch*)realloc(g_patch_list,sizeof(struct model_patch) * (g_path_num+1));

     g_patch_list[g_path_num] = *patch;
     g_path_num++;
}
