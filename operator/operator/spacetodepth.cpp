...
bool SpaceToDepth::InferShape(const std::vector<TEngine::TShape>& ishape, std::vector<TEngine::TShape>& oshape,
                              int layout)
{
...
{
    Input({"input:float32"}).Output({"output:float32"}).SetAttr("block_size", 1);
}
