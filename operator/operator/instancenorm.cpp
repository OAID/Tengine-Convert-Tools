...
bool InstanceNorm::InferShape(const std::vector<TEngine::TShape>& ishape, std::vector<TEngine::TShape>& oshape,
                              int layout)
{
...
{
    Input({"input:float", "gamma:float", "beta:float"}).Output({"output:float"}).SetAttr("eps", 0.001f);
}
