...
bool Pooling::InferShape(const std::vector<TEngine::TShape>& ishape, std::vector<TEngine::TShape>& oshape, int layout)
{
    ...
    int tmp_w0 = param_.pad_w0;
    int tmp_w1 = param_.pad_w1;
    int tmp_h0 = param_.pad_h0;
    int tmp_h1 = param_.pad_h1;
    ...
    const char* cal_str_uint8 = std::getenv("ENABLE_YOLO_TINY_UINT8");
    if (cal_str_uint8)
    {
        param_.pad_w0 = tmp_w0;
        param_.pad_w1 = tmp_w1;
        param_.pad_h0 = tmp_h0;
        param_.pad_h1 = tmp_h1;
    }
    TShape shape;
