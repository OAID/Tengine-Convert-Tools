...
bool Hardsigmoid::InferShape(const std::vector<TEngine::TShape>& ishape, std::vector<TEngine::TShape>& oshape,
                             int layout)
{
...
{
    Input({"input:float"}).Output({"output:float"}).SetAttr("alpha", 0.2f).SetAttr("beta", 0.5f);
}
